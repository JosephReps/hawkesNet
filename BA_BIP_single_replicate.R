...
if (length(args) < 2) stop("Usage: run_ba_bip_rep.R <rep_id> <out_dir>")
...
sim_out <- tryCatch({
  ...
  saveRDS(res, file = file.path(out_dir, sprintf("ba_bip_rep_%03d.rds", rep_id)))
...
if (!is.null(ev)) {
  td <- tryCatch(
    diag_temporal(ev = ev, params = params_true, T_end = T_end, T0 = 0),
...
res$lambda_new_hat <- p_hat$lambda_new %||% NA_real_
